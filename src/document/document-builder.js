    ...
    if (/\.md$/.test(doc)) {
      ...
      readLine.on("close", () => {
        Object.keys(document).forEach((key) => {
          ...
          outStream.write(`import { ElDocument } from "../";` + os.EOL);
          if (document[key].attributes) {
            outStream.write(
              `import { DocumentAttribute } from "../";` + os.EOL
            );
          }
          if (document[key].methods) {
            outStream.write(`import { DocumentMethod } from "../";` + os.EOL);
          }
          if (document[key].events) {
            outStream.write(`import { DocumentEvent } from "../";` + os.EOL);
          }
          if (document[key].slots) {
            outStream.write(`import { DocumentSlot } from "../";` + os.EOL);
          }
          if (document[key].scopedSlots) {
            outStream.write(
              `import { DocumentScopedSlot } from "../";` + os.EOL
            );
          }
